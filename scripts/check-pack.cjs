...
for (const packageRoot of packageRoots) {
  ...
  for (const file of pack.files || []) {
    ...
    if (filePath.includes("/node_modules/") || filePath.startsWith("node_modules/") || filePath.endsWith(".tgz")) {
      console.error(`Unsafe file in PugDoc pack dry-run: ${file.path}`);
      failed = true;
    }
  }
  if (packageJson.name === "@hia-doc/pugdoc-runner" || packageJson.name === "@hia-doc/pugdoc-producer") {
  ...
  if (packageJson.name === "@hia-doc/pugdoc-runner" && !packedPaths.has("src/cli.mjs")) {
    console.error("Missing src/cli.mjs in @hia-doc/pugdoc-runner pack dry-run.");
    failed = true;
  }
}
...
console.log(`PugDoc pack check passed: ${packageRoots.length} workspace packages.`);
